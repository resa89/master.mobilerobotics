...
for k = 1:Nstate
    % control vector
    uRausch = 0.95*uRausch + 0.05*[randn(1)*sAbweichV randn(1)*sAbweichOmega]';
...
scatter(xRef(1,:), xRef(2,:),5,'MarkerFaceColor','b'); %, cos(x_ref(3,:))*v, sin(x_ref(3,:))*v);

% scatter(xMess(1,:), xMess(2,:),'MarkerEdgeColor',[0 0.5 0],'MarkerFaceColor',[0 0.5 0]); %, cos(x(3,:))*v, sin(x(3,:))*v, 'r');
plot(xMess(1,:), xMess(2,:),'r'); %, cos(x(3,:))*v, sin(x(3,:))*v, 'r');
scatter(xOdom(1,:), xOdom(2,:),5,'MarkerEdgeColor','g','MarkerFaceColor','g'); %, cos(x(3,:))*v, sin(x(3,:))*v, 'r');

scatter(l(1,:), l(2,:),'MarkerEdgeColor',[0 0.5 0],'MarkerFaceColor',[0 0.5 0]); %, cos(x(3,:))*v, sin(x(3,:))*v, 'r');
